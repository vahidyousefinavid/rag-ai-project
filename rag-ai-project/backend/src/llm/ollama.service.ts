...
export class OllamaService {
  ...
  constructor() {
    ...
    // Structured/longer-output tasks (e.g. data profiling) need more room than a short chat reply.
    this.analyzer = new Ollama({
      baseUrl: BASE_URL,
      model: MODEL,
      temperature: 0.1,
      numPredict: 700,
    ...
    // Chat-style model for tool-calling (the agent): needs .bindTools(), which the
  ...
  async analyze(prompt: string): Promise<string> {
    return this.analyzer.invoke(prompt);
  }
