...
export class VectorService {
  ...
  /** Distinct (url, title) pairs actually indexed for a source — the ground-truth list of crawled pages. */
  async listPagesForSource(sourceId: string, limit = 300): Promise<{ url: string; title: string }[]> {
    ...
    try {
      while (seen.size < limit) {
        ...
        for (const point of res.points) {
          ...
          if (metadata.url && !seen.has(metadata.url)) seen.set(metadata.url, metadata.title ?? '');
        }

        if (!res.next_page_offset || res.points.length === 0) break;
